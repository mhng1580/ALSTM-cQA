...
if __name__=="__main__":
    ...
    revs_dev, vocab, keys_dev = build_data_cv(xml_filePath_test, CV_info, specialStr = "<tofixlength>", clean_string = True, mFold = foldNo, vocab = vocabA, test = True)
    revsC_dev, vocabC, keysC_dev = build_data_cv_taskC(xml_filePath_test, CV_info, specialStr = "<tofixlength>", clean_string = True, mFold = foldNo, vocab = vocabA, test = True)
    revsA_dev, vocabA, keysA_dev = build_data_cv_taskA(xml_filePath_test, CV_info, specialStr = "<tofixlength>", clean_string = True, mFold = foldNo, vocab = vocabA, test = True)
    ...
    # TaskB
    x1,x2,y,cv = packData(revs_D, W, W2, word_idx_map, word_idx_map, keysD)
    cPickle.dump([x1,x2,y,word_idx_map,W,W2,keysD,cv], open("cqa_taskD.p." + name + '.'  + str(foldNo), "wb"))
 
    x1,x2,y,cv = packData(revs_dev, W, W2, word_idx_map, word_idx_map, keys_dev)
    cPickle.dump([x1,x2,y,word_idx_map,W,W2,keys_dev,cv], open("cqa_taskD_dev.p." + name + '.'  + str(foldNo), "wb"))
 

    # TaskMT
    orgQ,relQ,relC, y, cv = packDataU(revsU, W, W2, word_idx_map, word_idx_map, keysU)
    cPickle.dump([orgQ,relQ,relC,y,word_idx_map,W,W2,keysU,cv], open("cqa_taskU.p." + name + '.'  + str(foldNo), "wb"))

    orgQ,relQ,relC,y,cv = packDataU(revsU_dev, W, W2, word_idx_map, word_idx_map, keysU_dev)
    cPickle.dump([orgQ,relQ,relC,y,word_idx_map,W,W2, keysU_dev,cv], open("cqa_taskU_dev.p." + name + '.' + str(foldNo), "wb"))




    # TaskB
    x1,x2,y,cv = packData(revs, W, W2, word_idx_map, word_idx_map, keys)
    cPickle.dump([x1,x2,y,word_idx_map,W,W2,keys,cv], open("cqa_taskB.p." + name + '.'  + str(foldNo), "wb"))
 
    x1,x2,y,cv = packData(revs_dev, W, W2, word_idx_map, word_idx_map, keys_dev)
    cPickle.dump([x1,x2,y,word_idx_map,W,W2,keys_dev,cv], open("cqa_taskB_dev.p." + name + '.'  + str(foldNo), "wb"))
 
    # TaskA
    xC1,xC2,yC,cv = packData(revsC, W, W2, word_idx_map, word_idx_map, keysC)
    cPickle.dump([xC1,xC2,yC,word_idx_map,W,W2, keysC,cv], open("cqa_taskC.p." + name + '.'  + str(foldNo), "wb"))

    xC1,xC2,yC,cv = packData(revsC_dev, W, W2, word_idx_map, word_idx_map, keysC_dev)
    cPickle.dump([xC1,xC2,yC,word_idx_map,W,W2, keysC_dev,cv], open("cqa_taskC_dev.p." + name + '.'  + str(foldNo), "wb"))

    # TaskC
    xC1,xC2,yC,cv = packData(revsA, W, W2, word_idx_map, word_idx_map, keysA)
    cPickle.dump([xC1,xC2,yC,word_idx_map,W,W2, keysA,cv], open("cqa_taskA.p." + name + '.'  + str(foldNo), "wb"))

    xC1,xC2,yC,cv = packData(revsA_dev, W, W2, word_idx_map, word_idx_map, keysA_dev)
    cPickle.dump([xC1,xC2,yC,word_idx_map,W,W2, keysA_dev,cv], open("cqa_taskA_dev.p." + name + '.' + str(foldNo), "wb"))
