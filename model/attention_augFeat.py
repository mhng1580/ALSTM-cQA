...
class ALSTM_augFeat_model(object):
    def __init__(self, n_word, n_in, n_lstm, n_attHidden, n_hidden, n_out, augFeatDim, ifDropout, drVal=0.2, l2Decay=1e-3, embVal=None, ifFixEmb=False, ifRegEmb=True):
        ...
        self.h = HiddenLayer(n_lstm * 2 + augFeatDim, n_hidden, prefix='h')
    ...
    def get_mlpInput(self, input1, inputMask1, input2, inputMask2, augInput):
        ...
        sentVec2 = self.dp.get_output(self.enc2.get_output(inputEmb2, inputMask2, sentVec1, sentCell1)[0][-1,:,:],
                                      self.ifDropout)
        mlpInput = T.concatenate([self.att.get_output(encOutputs1, inputMask1, sentVec2), augInput], axis=1)
        return mlpInput

    def get_hOutput(self, input1, inputMask1, input2, inputMask2, augInput):
        inputEmb1 = self.dp.get_output(self.emb.get_output(input1), self.ifDropout)
        inputEmb2 = self.dp.get_output(self.emb.get_output(input2), self.ifDropout)
    ...
    def get_output(self, input1, inputMask1, input2, inputMask2, augInput):
        hOutput = self.get_hOutput(input1, inputMask1, input2, inputMask2, augInput)
        return self.s.get_output(hOutput)

    def get_cost(self, input1, inputMask1, input2, inputMask2, y, augInput):
        hOutput = self.get_hOutput(input1, inputMask1, input2, inputMask2, augInput)
        return T.mean(self.s.negative_log_likelihood(hOutput, y)) + self.L2
    
    def get_loss(self, input1, inputMask1, input2, inputMask2, y, augInput):
        hOutput = self.get_hOutput(input1, inputMask1, input2, inputMask2, augInput)
        return self.s.negative_log_likelihood(hOutput, y)

    def get_errors(self, input1, inputMask1, input2, inputMask2, y, augInput):
        hOutput = self.get_hOutput(input1, inputMask1, input2, inputMask2, augInput)
